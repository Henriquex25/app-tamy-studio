...
export default function HomeScreen() {
    ...
    const [showAppointmentCancellationConfirmationModal, setShowAppointmentCancellationConfirmationModal] =
    ...
    async function loadMoreAppointments(): Promise<void> {
        ...
        if (hasMoreAppointments) {
            ...
            if (result.status === 200) {
                ...
                const resultAppointments: AppointmentType[] = result.data.data.filter(
        ...
        try {
            ...
            if (response.status === 200) {
                const filteredAppointments: AppointmentType[] = appointments.filter(
                    (appointment: AppointmentType): boolean => appointment.id !== id
                );
    ...
    function MenuAnchor(id: string | number): ReactElement {
        return (
            <TouchableOpacity onPress={() => visibleMenu(id, true)} activeOpacity={0.6}>
                <Ionicons name="ellipsis-vertical-sharp" size={26} color={themeColors.primary[400]} />
            </TouchableOpacity>
        );
    }
    ...
    const LoadingIndicator = (): ReactElement => {
        return <ActivityIndicator color={themeColors.primary[400]} size="large" />;
    };
            ...
            <View className="mt-1.5 mb-[8rem] py-3">
                {loadingAppointments ? (
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            <View
                                className="w-[89%] rounded-xl mx-auto flex flex-row bg-primary-100 overflow-hidden mb-5"
                                style={{
                                    shadowColor: "black",
                                ...
                                <View className="px-5 py-2 flex flex-row">
                                    ...
                                    <View className="w-7/12">
                                        ...
                                        <Text className="text-gray-500 mx-auto font-semibold">
                                            {`${item.scheduled_at.substring(0, 10).trim()} - ${item.scheduled_at
                                                .substring(10)
                                                .trim()}`}
                                        </Text>
                                    </View>
                                    ...
                                    <View className="w-4/12 flex flex-col">
                                        ...
                                        <View className="flex flex-row pr-1.5 w-full justify-end gap-x-1">
                                            ...
                                            <Text className="text-center text-lg font-bold text-gray-400">
                                                {item.service.formatted_price.substring(3).trim()}
                                            </Text>
                                        </View>
                        ...
                        ListEmptyComponent={
                            <Text className="text-center text-gray-600">Nenhum agendamento encontrado.</Text>
                        }
                        onEndReached={loadMoreAppointments}
                        onEndReachedThreshold={0.1}
                        ListFooterComponent={loadingMoreAppointments ? <LoadingIndicator /> : undefined}
                    />
                )}
            </View>
